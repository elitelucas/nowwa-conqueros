...
let Compile = async (): Promise<void> => {
    ...
    try {
        ...
        if (env == 'dev') {
            ...
            commands.push(`parcel watch ./src/NewPages/Index.html --dist-dir ./storage --no-content-hash`);
        } else if (env == 'prod') {
            commands.push(`npx tsc`);
            commands.push(`parcel build ./src/NewPages/Index.html --dist-dir ./storage`);
